...
import {
  ...
  PopoverContent,
  PopoverTrigger,
} from "src/components/ui/popover"; // Adjust path as needed
import { useGuestRoomStore } from "app/store/useGuestRoomStore"; // We will create this store

const GuestRoomSelector = () => {
  const { adults, children, infants, rooms, setAdults, setChildren, setInfants, setRooms } = useGuestRoomStore();
  const [isPopoverOpen, setIsPopoverOpen] = useState(false);

  const handleIncrement = (type: 'adults' | 'children' | 'infants' | 'rooms') => {
  ...
  const handleDecrement = (type: 'adults' | 'children' | 'infants' | 'rooms') => {
  ...
  const displayValue = `${adults + children + infants} Guest(s), ${rooms} Room(s)`;
